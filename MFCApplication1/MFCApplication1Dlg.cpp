...
BEGIN_MESSAGE_MAP(CMFCApplication1Dlg, CDialogEx)
	...
	ON_COMMAND(ID_mu_reset, &CMFCApplication1Dlg::OnMuReset)
END_MESSAGE_MAP()
...
void CMFCApplication1Dlg::tracktarget(LPVOID args)
{
	CMFCApplication1Dlg* dlg = (CMFCApplication1Dlg*)args;
	...
	for (int i = 0; i<cap_count; i++)
	{
		...
		if (dlg->resetflag)
		{
			delete(cap);
			cap = NULL;
			//WaitForSingleObject(recthandle, 0);
			//CloseHandle(recthandle);


			dlg->resetflag = false;
			dlg->updaterect2dflag = false;
			//WaitForSingleObject(tracktarget, INFINITE);
			//CloseHandle(tracktarget);
			dlg->ClearControl();
			break;
		}
		...
		*cap >> dlg->srcImage;
		...
		//两个线程不同步会产生误差积累，时间越长误差越大，追踪漂移越大
		if (frame_num % 10 == 0)
		{
			//imgindex = frame_num;
			//vadd.push_back(frame_num);
			dlg->updateflag = true;
		}
		else
		...
		if (firstflag)
		{
			...
			dlg->updaterect2dflag = true;
			recthandle = CreateThread(NULL, 0, LPTHREAD_START_ROUTINE(updaterect2d), dlg, 0, NULL);
	...
	dlg->updaterect2dflag = false;
	DWORD dwRet = WaitForSingleObject(recthandle, INFINITE);
	if (dwRet == WAIT_OBJECT_0)
		AfxMessageBox(_T("线程recthandle执行结束"));
	else if (dwRet == WAIT_TIMEOUT)
		AfxMessageBox(_T("线程recthandle等待超时"));
	else if (dwRet == WAIT_ABANDONED)
		AfxMessageBox(_T("线程recthandle尚未执行"));
	else
		AfxMessageBox(_T("线程recthandle未返回"));
...
void CMFCApplication1Dlg::OnMuMatch()
{
	...
	/*******************测试************************/
	//VideoCapture capp;
	//capp.open(FilePathName);
	////namedWindow("dfs", CV_WINDOW_NORMAL);
	//while (capp.isOpened())
	//{
	//	capp >> srcImage;
	//	//imshow("dfs", srcImage);
	...
	if (b_num != 0 && dlg.DoModal() == IDOK)
	{
		...
		HANDLE trackhandle;
		trackhandle = CreateThread(NULL, 0, LPTHREAD_START_ROUTINE(tracktarget), this, 0, NULL);
		/*DWORD dwRet = WaitForSingleObject(trackhandle, INFINITE);
		if (dwRet == WAIT_OBJECT_0)
			AfxMessageBox(_T("创建的线程执行结束"));
		else if (dwRet == WAIT_TIMEOUT)
			AfxMessageBox(_T("等待超时"));
		else if (dwRet == WAIT_ABANDONED)
			AfxMessageBox(_T("尚未执行"));
		else
			AfxMessageBox(_T("未返回"));*/
		/*************************************************************/
	}
}
...
void CMFCApplication1Dlg::ShowToControl(Mat stc_img)
{
	...
	//CDC* pDC = new CDC;
	//pDC = GetDlgItem(IDC_STATICPicture)->GetDC();
	HDC hdc = GetDlgItem(IDC_STATICPicture)->GetDC()->GetSafeHdc();
	CRect rect;
	// 矩形类
	...
	//frame = cvCloneImage(&frametemp);//cvCloneImage函数会导致内存泄漏，应将其替换为cvCopy()
	//frame = cvCreateImage(desImage.size(), IPL_DEPTH_8U, desImage.channels());
	//cvCopy(&frametemp, frame, NULL);
	//cimg.CopyOf(frame, frame->nChannels);
	cimg.CopyOf(&frametemp, frametemp.nChannels);
...
void CMFCApplication1Dlg::ReadImgData(int &b_num)
{
	...
	if (filedlg.DoModal() == IDOK)
	{
		...
		{
			//读取图片编号，注意，图片编号不能太长，否则会被认为是两个字符串
...
void CMFCApplication1Dlg::getbox(vector<Img_Data> v_data, int box_i, vector<Point2f> &cur_corners, vector<Box> cur_ol_corner)
{
	...
	cur_corners = scene_corners;
	//rect原点的问题，原点选取不正确，有时候0有时候1
	double rect_x = cur_corners[0].x;
	double rect_y = cur_corners[0].y;
...
void CMFCApplication1Dlg::OnMuReset()
{
	// TODO: 在此添加命令处理程序代码
